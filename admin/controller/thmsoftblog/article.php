...
class ControllerThmsoftblogArticle extends Controller {
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmsoftblog_article->editArticle($this->request->get['blog_id'], $this->request->post);
			...
			$this->response->redirect($this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . $url, true));
		}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $blog_id) {
				$this->model_thmsoftblog_article->deleteArticle($blog_id);
			}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . $url, true)
		);
		
		$data['add'] = $this->url->link('thmsoftblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('thmsoftblog/article/delete', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			...
			$data['articles'][] = array(
				...
				'edit'       => $this->url->link('thmsoftblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $result['blog_id'] . $url, true)
...
//print_r($data['articles']);exit();
		...
		$data['sort_name'] = $this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . '&sort=md.name' . $url, true);
		$data['sort_status'] = $this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . '&sort=m.status' . $url, true);
		...
		$pagination->url = $this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
		...
		$data['entry_title_relatedproducts'] = $this->language->get('entry_title_relatedproducts');
		$data['entry_related'] = $this->language->get('entry_related');
		$data['help_related'] = $this->language->get('help_related');

		$data['tab_relatedarticles'] = $this->language->get('tab_relatedarticles');
		$data['entry_title_relatedarticles'] = $this->language->get('entry_title_relatedarticles');
		$data['entry_relatedarticles'] = $this->language->get('entry_relatedarticles');
		$data['help_relatedarticles'] = $this->language->get('help_relatedarticles');
		...
		if (isset($this->error['name'])) { $data['error_article'] = $this->error['name'];
		...
		if (!isset($this->request->get['blog_id'])) {
			$data['action'] = $this->url->link('thmsoftblog/article/add', 'token=' . $this->session->data['token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('thmsoftblog/article/edit', 'token=' . $this->session->data['token'] . '&blog_id=' . $this->request->get['blog_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'] . $url, true);

		$this->load->model('thmsoftblog/article');

		if (isset($this->request->get['blog_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$article_info = $this->model_thmsoftblog_article->getBlogarticle($this->request->get['blog_id']);
			$article_keyword_info = $this->model_thmsoftblog_article->getBlogarticlekeyword($this->request->get['blog_id']);
		}
		...
		if (isset($this->request->post['article_description'])) {
			...
			$data['article_description'] = $this->model_thmsoftblog_article->getarticleDescriptions($this->request->get['blog_id']);
		} else {
		...
		if (isset($this->request->post['article_category'])) { 
			...
			$data['article_category'] = $this->model_thmsoftblog_article->getBlogarticlecategory($this->request->get['blog_id']);
		...
		if (isset($this->request->post['article_store'])) { 
			...
			$data['article_store'] = $this->model_thmsoftblog_article->getBlogarticleStores($this->request->get['blog_id']);
		...
		} elseif (!empty($article_keyword_info)) {
			$data['keyword'] = $article_keyword_info['keyword'];
		} else {
			$data['keyword'] = '';
		}
		...
		if (isset($this->request->post['related_product'])) {
			...
			$products = $this->model_thmsoftblog_article->getRelatedProduct($this->request->get['blog_id']);
...
// related articles
		if (isset($this->request->post['related_article'])) {
			$related_article = $this->request->post['related_article'];
		} elseif (isset($this->request->get['blog_id'])) {
			$related_article = $this->model_thmsoftblog_article->getRelatedArticle($this->request->get['blog_id']);
		...
		foreach ($related_article as $article_id) {
		
		$related_article_info = $this->model_thmsoftblog_article->getarticleDescriptions($article_id);

		if ($related_article_info) {
		$data['related_articles'][] = array(
		'blog_id' => $article_id,
		'name' => $related_article_info[(int)$this->config->get('config_language_id')]['name'],
		...
		$this->response->setOutput($this->load->view('thmsoftblog/article_form', $data));
	}
	public function add() { 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) { 
			$this->model_thmsoftblog_article->addArticle($this->request->post);		
			
			$this->session->data['success'] = $this->language->get('text_success');
						
			$this->response->redirect($this->url->link('thmsoftblog/article', 'token=' . $this->session->data['token'], true));
		}
	...
	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'thmsoftblog/article')) {
			$this->error['warning'] = $this->language->get('error_permission');
		}
		foreach ($this->request->post['article_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 255)) {
				$this->error['name'][$language_id] = $this->language->get('error_article');
			}
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			if ($url_alias_info && isset($this->request->get['blog_id']) && $url_alias_info['query'] != 'thmblogarticle_id=' . $this->request->get['blog_id']) {
